...
YostLabDriver::YostLabDriver(ros::NodeHandle& nh_, ros::NodeHandle& priv_nh_) : SerialInterface(priv_nh_),
...
const std::string YostLabDriver::getSoftwareVersion() {
  ...
  ROS_INFO_STREAM(this->logger << "Firmware version: " << buf);
  return buf;
}
...
void YostLabDriver::run() {
  this->getSoftwareVersion();
  // this->SerialWriteString(SET_GYRO_DISABLED);
  // this->SerialWriteString(SET_ACCELEROMETER_DISABLED);
  // this->SerialWriteString(SET_COMPASS_ENABLED);
  // this->SerialWriteString(SET_AXIS_DIRECTION);
  // this->SerialWriteString(SET_EULER_ANGLE_DECOMP_ORDER_XYZ);
  // this->SerialWriteString(SET_OVERSAMPLE_RATE);
  // this->SerialWriteString(SET_RUNNING_AVERAGE);
  // this->SerialWriteString(SET_COMPASS_RANGE);
  // this->SerialWriteString(SET_REFERENCE_VECTOR_MODE);
  // this->SerialWriteString(SET_COMPASS_CALIBRATION_COEFFICIENTS);
  this->SerialWriteString(SET_STREAMING_SLOTS_COMPASS);
  ...
  while (ros::ok()) {
    while (this->Available() > 0) {
      ...
      if (line_num_ == 3) {
        ...
        imu_msg_.header.stamp = ros::Time::now();
        imu_msg_.header.frame_id = "imu_link";
        // for now use orientation to hold x,y,z,yaw rather than quaternion
        imu_msg_.orientation.x = parsed_val_[0];
        imu_msg_.orientation.y = parsed_val_[1];
        imu_msg_.orientation.z = parsed_val_[2];
        double yaw = atan2(imu_msg_.orientation.y, imu_msg_.orientation.x);
        imu_msg_.orientation.w = yaw;
        ROS_INFO("[ YostLabImuDriver ] yaw: %.3f = %.3f, x: %.3f, y: %.3f, z: %.3f ", yaw, yaw * 180.0 / 3.14159265, parsed_val_[0], parsed_val_[1], parsed_val_[2]);
        parsed_val_.clear();
        this->imu_pub_.publish(imu_msg_);
      }
    }
